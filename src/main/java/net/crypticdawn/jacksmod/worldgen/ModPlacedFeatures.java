package net.crypticdawn.jacksmod.worldgen;
...
public class ModPlacedFeatures {
    ...
    public static void bootstrap(BootstapContext<PlacedFeature> context) {
        // references ModConfiguredFeatures - looks up via resource key
        HolderGetter<ConfiguredFeature<?, ?>> configuredFeatures = context.lookup(Registries.CONFIGURED_FEATURE);

        // specify veins per chunk (pCount), uniform vs triangle etc., heightRangePlacement y level eg -64, 80
    ...
    private static ResourceKey<PlacedFeature> registerKey(String name) {
        return ResourceKey.create(Registries.PLACED_FEATURE, new ResourceLocation(JacksMod.MOD_ID, name));
    }
